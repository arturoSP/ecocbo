...
assempar_mod <- function(data, type = c("P/A", "counts", "cover"), Sest.method = "average") {
  ...
  if (type == "P/A") {
    ...
    if (Par$Sest > Par$Sobs) {
      ...
      Par$par[, 1] <- c(sp.names, paste("unseen species", c(1:(Par$Sest - Par$Sobs)))) # unseen species are now numbered
  ...
  if (type == "counts") {
    ...
    if (Par$Sest > Par$Sobs) {
      ...
      # Par$par[, 1] <- c(sp.names, rep("unseen species", Par$Sest - Par$Sobs))
      Par$par[, 1] <- c(sp.names, paste("unseen species", c(1:(Par$Sest - Par$Sobs)))) # unseen species now are numbered
      Par$par[, 2] <- c(fs, rep(min(fs, na.rm = TRUE), Par$Sest - Par$Sobs))
      Par$par[, 3] <- c(fw, rep(min(fw, na.rm = TRUE), Par$Sest - Par$Sobs))
