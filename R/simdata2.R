...
#'
#' Simulates multiple ecological data sets using parameters estimated from a pilot
#' study (see \code{assempar}). The generated data preserve key destributional
#' properties of the pilot assemblage, and can be used in downstream SSP functions
#' for quality evaluation and sampling effort estimation. It is possible to add a
#' small, controlled jitter to among-site detection probabilities to induce subtle
#' differences among simulated communities.
#'
#' @param Par A list of parameters estimated by \code{assempar}.
...
#' @param jitter.base Numeric scalar in \eqn{\[0,1\)}. Standard deviation multiplier
#' used to add Gaussian jitter to \code{fs} and \code{fw}. Defaults to 0.
#'
#' @details
#' Presence/absence data are simulated using Bernoulli trials based on empirical
#' frequencies of occurrence among sites (for site-level presence) and within sites
#' (for local occurrence patterns). These matrices are then converted into abundance
#' matrices using values drawn from Poisson or negative binomial distributions (for
#' count data), or from log-normal distributions (for continuous data like coverage
#' or biomass), depending on the aggregation properties estimated in the pilot data.
#'
#' This process is repeated \code{cases} times, producing a list of simulated data
#' sets that reflect the statistical properties of the original assemblage, but
#' without incorporating environmental constraints or species co-occurrence structures.
#'
#' @return A list of simulated community data sets, to be used by \code{datquality}
#' and \code{sampsd}.
#'
#' @note
#' This simulation assumes that differences in composition or abundance are due to
#' spatial aggregation, as captured by the pilot data. It does not incorporate
#' environmental gradients or species associations. For more advanced modeling of
#' species associations, copula-based approaches as suggested by Anderson et al.
#' (2019) may be integrated in future versions of SSP.
#'
#' When using a jitter.base, the simulation is stochastic. For reproducibility, use
...
#'
#' @seealso \code{sampsd}, \code{datquality}
#'
#' @importFrom stats rbinom rnbinom rnorm rpois
#' @keywords internal
#' @noRd
#'
