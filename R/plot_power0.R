...
power_curve <- function(powr, m = NULL, n, cVar, model) {
  ...
  if (model == "single.factor") {
    ...
    p1 <- ggplot(data = powrPl, aes(x = n, y = .data$Power)) +
      ...
      geom_point(size = 1) +
      geom_point(
        data = powrPl[powrPl$n == nn, ],
        shape = 21,
        size = 2,
        stroke = 1.5,
        fill = "white"
      ) +
      annotate("label", x = labx, y = 0.1, label = paste0("n = ", nn))
  } else {
    mTot <- c(2:max(powr$m))
    mm <- m
    nn <- n
    ...
    p1 <- ggplot(data = powrPl[powrPl$m == mm, ], aes(x = n, y = .data$Power)) +
      ...
      geom_point(
        data = powrPl[powrPl$m == mm & powrPl$n == nn, ],
        shape = 21,
        size = 2,
        stroke = 1.5,
        fill = "white"
...
#' @importFrom ggplot2 scale_x_continuous scale_y_continuous scale_color_manual
#' @importFrom ggplot2 scale_linetype_manual element_blank element_rect element_line
#' @importFrom ggplot2 theme guides geom_col geom_area geom_vline coord_cartesian geom_text
#' @importFrom ggplot2 annotate
#' @importFrom stats density
#' @importFrom rlang .data
#'
...
density_plot <- function(
  ...
  densHistH0 <- data.frame(
    x = histH0$mids,
    y = ((histH0$density - bottHist) / (topHist - bottHist))
  )
  densHistHa <- data.frame(
    x = histHa$mids,
  ...
  p1 <- ggplot() +
    ggplot2::geom_col(
      data = densHistHa,
      aes(x = densHistHa[, 1], y = densHistHa[, 2]),
      color = "#56B4E9",
      fill = "#56B4E9",
      alpha = 0.1
    ) +
    ggplot2::geom_col(
      data = densHistH0,
      aes(x = densHistH0[, 1], y = densHistH0[, 2]),
      color = "#E69F00",
      fill = "#E69F00",
      alpha = 0.1,
    ) +
  ...
  if (completePlot == FALSE) {
    ...
    ftop <- as.numeric(rownames(ftop[
      rank(ftop[, 2], ties.method = "first")[1],
    ]))
...
surface_plot <- function(powr, model) {
  if (model == "single.factor") {
    message(
      "The surface plot is available only for nested factors experiments."
    )
  } else {
    ...
    Z <- matrix(powr$Power, nrow = length(ns), ncol = length(ms), byrow = FALSE)
